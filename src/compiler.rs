...
mod vm_dump_tests {
  ...
  #[test]
  fn dump_for_loop_bytecode() {
    let src = std::fs::read_to_string("examples/for_loop.gup").unwrap();
    let tokens = lexer::lex(&src).unwrap();
    let prog = parser::parse(tokens).unwrap();
    let script = compile(&prog).unwrap();
    let chunk = &script.borrow().chunk;
    eprintln!("CODE: {:?}", chunk.code);
    let mut ip = 0usize;
    while ip < chunk.code.len() {
      let start = ip;
      let op = crate::bytecode::decode_opcode(chunk.code[ip]).unwrap();
      ip += 1;
      let mut detail = format!("{:?}", op);
      match op {
        crate::bytecode::OpCode::Constant
        | crate::bytecode::OpCode::GetLocal
        | crate::bytecode::OpCode::SetLocal
        | crate::bytecode::OpCode::DefineLocal
        | crate::bytecode::OpCode::GetGlobal
        | crate::bytecode::OpCode::SetGlobal
        | crate::bytecode::OpCode::DefineGlobal
        | crate::bytecode::OpCode::GetUpvalue
        | crate::bytecode::OpCode::SetUpvalue
        | crate::bytecode::OpCode::StoreVariable
        | crate::bytecode::OpCode::Call
        | crate::bytecode::OpCode::Closure
        | crate::bytecode::OpCode::Print
        | crate::bytecode::OpCode::MakeArray => {
          detail.push_str(&format!(" {}", chunk.code[ip]));
          ip += 1;
        }
        crate::bytecode::OpCode::Jump
        | crate::bytecode::OpCode::JumpIfFalse
        | crate::bytecode::OpCode::Loop => {
          let offset = ((chunk.code[ip] as u16) << 8) | chunk.code[ip + 1] as u16;
          detail.push_str(&format!(" -> {}", offset));
          ip += 2;
        }
        _ => {}
      }
      eprintln!("{:04} {}", start, detail);
    }
  }
}
