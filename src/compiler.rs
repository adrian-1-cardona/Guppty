...
impl Compiler {
    ...
    fn chunk_mut(&mut self) -> std::cell::RefMut<'_, Chunk> {
        std::cell::RefMut::map(self.contexts[self.current].function.borrow_mut(), |f| {
            &mut f.chunk
        })
    }
    ...
    fn end_scope(&mut self, span: Span) {
        ...
        loop {
            let should_pop = self.contexts[self.current]
                .locals
                .last()
            ...
            let is_captured = self.contexts[self.current]
                .locals
                .last()
                .unwrap()
                .is_captured;
    ...
    fn compile_expression(&mut self, expr: &Expr) -> Result<(), GupError> {
        match &expr.kind {
            ExprKind::StringLiteral(text) => {
                self.make_constant(Constant::Value(Value::GuppyString(text.clone())), expr.span);
            }
