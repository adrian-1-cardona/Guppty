...
impl Vm {
    ...
    fn run_loop(&mut self) -> Result<(), GupError> {
        loop {
            ...
            let instruction = decode_opcode(opcode_byte).ok_or_else(|| {
                GupError::runtime(Span::new(1, 1, 1), "Unknown opcode in bytecode.")
            })?;
            self.current_frame_mut().ip = ip + 1;
            ...
            match instruction {
                ...
                OpCode::GetGlobal => {
                    ...
                    let value = self.globals.get(&name).cloned().ok_or_else(|| {
                        self.runtime_error(format!("Variable '{}' is not defined yet!", name))
                    })?;
                    self.push(value);
                }
                OpCode::SetGlobal => {
    ...
    fn read_upvalue(&self, slot: usize) -> Result<Value, GupError> {
        ...
        if let Some(open_slot) = upvalue.open_slot {
            ...
            Ok(upvalue.closed.clone().unwrap_or(Value::Nothing))
        }
    }
    ...
    fn build_range(&self, start_val: Value, end_val: Value) -> Result<Value, GupError> {
        ...
        let end_num = end_val.as_number().map_err(|msg| self.runtime_error(msg))?;
    ...
    fn call_value(&mut self, arg_count: usize) -> Result<(), GupError> {
        ...
        match callee {
            ...
            other => Err(self
                ...
                .with_hint(
                    "Only call functions defined with a function; check the value before '('.",
                )),
        }
    }
    ...
    fn make_closure(&mut self, constant: u8) -> Result<(), GupError> {
        let function =
            match &self.current_frame().function.borrow().chunk.constants[constant as usize] {
        ...
        self.push(Value::GuppyClosure(ClosureValue { function, upvalues }));
        Ok(())
    }
...
mod tests {
    ...
    fn top(stack: &[Value]) -> &Value {
        stack
            .last()
            .expect("expected at least one value on the stack")
    }
    ...
    fn subtracts_multiplies_and_divides() {
        ...
        sub.push_value(Value::GuppyNumber(10))
        ...
        mul.push_value(Value::GuppyNumber(6))
        ...
        div.push_value(Value::GuppyNumber(20))
    ...
    fn comparisons_push_booleans() {
        ...
        less.push_value(Value::GuppyNumber(2))
            .push_value(Value::GuppyNumber(3))
            .op(OpCode::Less);
    ...
    fn add_concatenates_when_a_string_is_involved() {
        ...
        both.push_value(Value::GuppyString("foo".to_string()))
    ...
    fn build_range_counts_up_and_down() {
        ...
        down.push_value(Value::GuppyNumber(3))
            .push_value(Value::GuppyNumber(1))
            .op(OpCode::BuildRange);
        assert_eq!(
